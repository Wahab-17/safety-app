...
class _DashboardState extends State<Dashboard> {
  ...
  final List<Widget> screens = [const Home(), const MyContactsScreen()];
  bool alerted = false;
  int currentPage = 0;
    ...
    return Scaffold(
      ...
      floatingActionButton: currentPage == 1
          ...
          : FloatingActionButton(
              ...
              onPressed: () async {
                if (alerted) {
                  final int pin = (prefs?.getInt('pin') ?? -1111);
                  if (pin == -1111) {
                    sendAlertSMS(false);
                  } else {
  ...
  checkPermission() async {
    ...
    permissions.PermissionStatus conPer =
        await permissions.Permission.contacts.status;
    permissions.PermissionStatus locPer =
        await permissions.Permission.location.status;
    permissions.PermissionStatus phonePer =
        await permissions.Permission.phone.status;
    permissions.PermissionStatus smsPer =
        await permissions.Permission.sms.status;
    if (conPer != permissions.PermissionStatus.granted) {
      await permissions.Permission.contacts.request();
    }
    if (locPer != permissions.PermissionStatus.granted) {
      await permissions.Permission.location.request();
    }
    if (phonePer != permissions.PermissionStatus.granted) {
      await permissions.Permission.phone.request();
    }
    if (smsPer != permissions.PermissionStatus.granted) {
      await permissions.Permission.sms.request();
    }
  }
